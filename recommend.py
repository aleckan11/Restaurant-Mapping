...
def group_by_centroid(restaurants, centroids):
    ...
    for i in restaurants:
        clusters += [[find_closest(restaurant_location(i), centroids),i]]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for i in feature_fns:
        predictors_and_r += [find_predictor(user, reviewed, i)]
    r_squared = []
    for i in predictors_and_r:
        r_squared += [i[1]]
    return predictors_and_r[r_squared.index(max(r_squared))][0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for i in restaurants:
        if i in reviewed:
            dictionary[restaurant_name(i)] = user_rating(user, restaurant_name(i))
        else:
            dictionary[restaurant_name(i)] = predictor(i)
    return dictionary
    # END Question 9
